...
class ReadIMUTask:
    ...
    def __init__(self, imu, heading, heading_offset, read_IMU_flg, mtr_enable):
    ...
    def run(self):
        while True:
            if (self.state == self.S0_INIT):
                ...
                self.imu.set_operation_mode("ndof")  # set to fusion mode yaw offset
                ...
                if self.first_reading:
                    # On first reading, set heading offset
                    initial_heading = self.imu.read_euler_angles()[0]  # yaw angle in degrees
                    self.heading_offset.put(initial_heading)
                    # print("Initial IMU Heading:", initial_heading)
                    self.first_reading = False

                heading = self.imu.read_euler_angles()[0] # yaw angle in degrees
                heading -= self.heading_offset.get() # Subtract offset
                # print("IMU Heading:", heading, "Heading Offset:", self.heading_offset.get())
